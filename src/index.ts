...
const createIpcHandlers = () => {
  ...
  ipcMain.handle(
    "run-prompt",
    (event, engineName, prompt, modelsOptions, remixOptions) => {
      ...
      return AIEngineFactory.runPrompt(
        engine,
        event,
        prompt,
        modelsOptions,
  ...
  ipcMain.handle(
    ...
    (event, currentDirectory: string, interaction: InteractionRecord) => {
      return HistoryHandler.saveInteraction(currentDirectory, interaction);
    }
  );
  ipcMain.handle(
    "history-get-interactions",
    (event, currentDirectory: string, limit?: number) => {
      return HistoryHandler.getInteractions(currentDirectory, limit);
    }
  );
  ipcMain.handle("history-clear-history", (event, currentDirectory: string) => {
    return HistoryHandler.clearHistory(currentDirectory);
  });
...
app.whenReady().then(() => {
  ...
  apiKeyStorage
    ...
    .catch((err) => {
      log.error("Failed to initialize API key storage:", err);
    });
